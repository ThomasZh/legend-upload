...
#        ./crntb-adm.sh uninstall
        echo "Stopping upload.7x24hs.com service, begin killing ${PID}"
    ...
    if [ "$PID" != "" ] ; then
        echo "Starting upload.7x24hs.com service..."
        nohup python index.py -log_file_prefix=./log/upload.log &
        echo $! > pidfile    #record process id to file
        echo 'Startup upload.7x24hs.com service success!'
